...
const findAllReachableSchemaNodeIds = (startingSchemaNodeNames: String[]) => {
  startingSchemaNodeNames.forEach((startingSchemaNodeName) => dfs(schemaNodeIdByName.get(startingSchemaNodeName), 0))

  console.log('found all reachable nodes')

  visitedSchemaNodeNames = new Set<String>(
    Array.from(visitedIds).map((visitedId) => schemaNodeById.get(visitedId).name),
  )
}
...
const schema = readFileSync(path.join(GQL_ROOT_PATH, 'schema-original.graphql'), 'utf-8')
...
writeFileSync(path.join(GQL_ROOT_PATH, 'schema-operation-filtered.graphql'), printSchema(operationFilteredDocumentNode))

/**
 * Graph 생성
 */

const { schemaNodeById, schemaNodeIdByName } = generateNodes(operationFilteredDocumentNode)
...
const filteredAST = filterOnlyVisitedSchema(operationFilteredDocumentNode, visitedSchemaNodeNames)

const filteredSchema = printSchema(filteredAST)

writeFileSync(path.join(GQL_ROOT_PATH, 'schema-filtered.graphql'), filteredSchema)
