...
const loadQueries = () => {
    const path = (0, posix_1.join)(caller_configuration_parser_1.cofiguration.filters, 'Query.json');
...
const loadMutations = () => {
    const path = (0, posix_1.join)(caller_configuration_parser_1.cofiguration.filters, 'Mutation.json');
    console.log(`loading Mutation filter from ${path}`);
    return (0, fs_1.existsSync)(path) ? JSON.parse((0, fs_1.readFileSync)(path, 'utf-8')) : [];
};
exports.loadMutations = loadMutations;
const loadSubscriptions = () => {
    const path = (0, posix_1.join)(caller_configuration_parser_1.cofiguration.filters, 'Subscription.json');
